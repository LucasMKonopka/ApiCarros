...
class CarrosHelper{
  ...
  Future<Database> initDb() async{
    ...
    return await openDatabase(path, version: 1, onCreate: (Database db, int newVersion) async {
      await db.execute("CREATE TABLE $carroTable($idColumn INTEGER PRIMARY KEY, $marcaColumn TEXT, $modeloColumn TEXT, $anoColumn TEXT, $corColumn TEXT, $categoriaColumn TEXT, $imgColumn TEXT)");
    },
    );
  }
  ...
  Future<Carro> getCarro(int id) async{
    Database dbCarro = await db;
    List<Map> maps = await dbCarro.query(carroTable, columns: [idColumn, marcaColumn, modeloColumn, anoColumn,  corColumn, categoriaColumn, imgColumn], where: "$idColumn = ?", whereArgs: [id]);
    if(maps.isNotEmpty){
  ...
  Future<int> updateCarro(Carro carro) async{
    Database dbCarro = await db;
    return await dbCarro.update(carroTable, carro.toMap(), where: "$idColumn = ?", whereArgs: [carro.id]);
  }

  Future<int> deleteCarro(int id) async{
    Database dbCarro = await db;
    return await dbCarro.delete(carroTable, where: "$idColumn = ?", whereArgs: [id]);
  }
